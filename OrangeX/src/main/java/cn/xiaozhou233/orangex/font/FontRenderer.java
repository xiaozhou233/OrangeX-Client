...
public class FontRenderer extends CFont {
    ...
    public FontRenderer(String filePath, int size, int fontType, boolean antiAlias, boolean fractionalMetrics) {
        super(FontUtil.getFontFromTTF(
                        FontRenderer.class.getClassLoader().getResourceAsStream("fonts/" + filePath), size, fontType),
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        boolean render = true;
        double scaleFactor = 2.0D / getFontScale();
        x *= scaleFactor;
        y = (y - 3.0D) * scaleFactor;

        if (render) {
            GL11.glPushMatrix();
            GlStateManager.scale(1.0D / scaleFactor, 1.0D / scaleFactor, 1.0D / scaleFactor);
            ...
            for (int i = 0; i < size; i++) {
                ...
                if ((String.valueOf(character).equals("\247")) && (i < size)) {
                    ...
                    if (colorIndex < 16) {
                        ...
                        if (italic) {
                            GlStateManager.bindTexture(getTexItalicBold().getGlTextureId());
                            // GL11.glfunc_179144_i(GL11.GL_TEXTURE_2D,
                            // texItalicBold.getGlTextureId());
                            currentData = this.boldItalicChars;
                        } else {
                            GlStateManager.bindTexture(getTexBold().getGlTextureId());
                            // GL11.glfunc_179144_i(GL11.GL_TEXTURE_2D,
                        ...
                        if (bold) {
                            ...
                            GlStateManager.bindTexture(getTexItalic().getGlTextureId());
                            // GL11.glfunc_179144_i(GL11.GL_TEXTURE_2D,
    ...
    public int getStringWidth(String text) {
        ...
        return (int) ((width / 2) * getFontScale());
    }
    ...
    public int getStringWidthCust(String text) {
        ...
        return (int) (((width - this.charOffset) / 2) * getFontScale());
    }
    ...
    private BufferedImage boldItalicImage;

    private void setupBoldItalicIDs() {
        boldImage = generateFontImage(this.font.deriveFont(1), this.antiAlias, this.fractionalMetrics, this.boldChars);
        italicImage = generateFontImage(this.font.deriveFont(2), this.antiAlias, this.fractionalMetrics, this.italicChars);
        boldItalicImage = generateFontImage(this.font.deriveFont(3), this.antiAlias, this.fractionalMetrics, this.boldItalicChars);
        texBold = null;
        texItalic = null;
        texItalicBold = null;
    }

    private AbstractTexture getTexBold() {
        if (texBold == null) {
    ...
    private AbstractTexture getTexItalic() {
        if (texItalic == null) {
    ...
    private AbstractTexture getTexItalicBold() {
        if (texItalicBold == null) {
            texItalicBold = setupTexture(boldItalicImage);
            boldItalicImage = null;
        }
        return texItalicBold;
    }
